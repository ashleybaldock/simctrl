...
show_usage() {
    echo "Usage: simctrl <tag> {status|start|restart|stop}" >&2
}
...
simpath="$pathtosimutrans/$revision/simutrans/sim"
...
simutrans_start() {
    ...
    echo -n "Attempting to start simutrans instance: $tag."
    logger -t $tag Attempting to start server...

    # Start up the server
    if [ $1 = "start" ]; then
        su simutrans -s /bin/sh -c "( $simpath -server $port -server_id $server_id -server_name $server_name -server_comment $server_comment -debug $debug -lang en -objects $objects -load $save 2>&1 & echo \$! >&3 ) 3>$pidfile | logger -i -t $tag &"
    elif [ $1 = "restart" ]; then
        su simutrans -s /bin/sh -c "( $simpath -server $port -server_id $server_id -server_name $server_name -server_comment $server_comment -debug $debug -lang en -objects $objects 2>&1 & echo \$! >&3 ) 3>$pidfile | logger -i -t $tag &"
    fi
